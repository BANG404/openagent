...
- **Conversation details panel:** the ordinary desktop conversation's resizable right-side card owns Goal/Graph status, edited files, terminals, and groups. Its top-level tabs switch between those views; each tab restores its last meaningful selection and draft within the active conversation branch, while the selected tab and collapse request are also branch-scoped. Conditional tab mounting must not erase that state or jump to an arbitrary first item. Newly created flows, file-only activity, and chat-group tool calls open the panel automatically; configuration alone does not create an empty chat-group tab. The title-bar toggle collapses it with the same 180ms width curve as the conversation sidebar and leaves no interactive or visible track behind.
