...
The channel navigation starts directly with the platform list, uses the same
width and row geometry as the model-provider list, and identifies each platform
with its recognizable icon. It does not repeat the Channels title inside the
panel. The detail heading groups connection state with the primary enable
control through the shared settings status-toggle treatment also used by model
providers and MCP servers. The bundled platform artwork is sourced from Cherry
Studio at commit `16e2905fd30fbfe21d1c58651574dcc939b6fb30`.
