...
metadata, channel, and selected source before reusing it when it has the
current release source as its parent. If that dedicated branch is stale but its
release commit was generated by GitHub Actions, preparation regenerates the
same version from the current verified source with a lease-protected update;
an existing release PR follows the updated head. A manually authored stale
branch stops preparation instead of being overwritten. An existing immutable
release tag stops preparation. The generated `prepare/v*` pull request CI
bypasses change detection and module checks, then immediately succeeds through
the always-present aggregate. Release preparation already requires a successful
source CI run, the generated commit contains only release metadata, and the
post-merge CI plus Release workflow validate and build the exact release commit.
The bypass applies only to same-repository preparation branches; similarly
named branches from forks use the ordinary pull-request checks. Preparation
still explicitly dispatches CI because events created with `GITHUB_TOKEN` do
not trigger another workflow; that trusted branch dispatch takes the same fast
bypass path.
The trusted PR-head reporter accepts the successful CI only for a same-repository
...
- Generated release metadata PRs bypass module checks because their source SHA
  was already verified before preparation. After merge, the integration tier
  verifies every fast module, release metadata validation binds the release
  commit to that source SHA, and complete product compilation remains owned by
