...
`prepare/v*` branch. After the authoritative result succeeds, it resolves the
single open release PR at that exact head, verifies the GitHub Actions bot author
and the Beta or Stable target derived from the versioned branch, publishes
`Required PR Head`, and squash-merges with the configured administrator token.
Starting `Prepare Release` is therefore the release authorization point; a valid
generated release PR does not require a second manual review or merge action.
...
If automatic merge is unavailable, review and squash-merge the PR only after
`CI / Required` and `Required PR Head` pass.
...
may automatically squash-merge either a ready PR authored by the repository
owner or a release PR generated by `Prepare Release`. It re-reads the PR and
requires an open non-draft state and the exact validated head SHA. Owner-authored
PRs must target the default branch. Generated release PRs must come from the same
repository, be authored by `github-actions[bot]`, use the exact `prepare/v*`
branch that was dispatched, and target either `master` for a Beta or the matching
`release/stable/X.Y.Z` branch for a Stable. The optional
