...
**`application-menu-bar`** — File, Edit, View, and Help share the compact desktop menu scale and remain available on every ordinary application surface. Access keys, arrow navigation, and platform-labeled accelerators invoke the same actions as pointer selection. Edit preserves the focused editable context for Undo, Redo, Cut, Copy, Paste, Delete, and Select All; Help exposes the shared updater state and disables duplicate checks. Workspace and conversation navigation always reuse the current window. File -> New window is the sole ordinary UI action that asks for a workspace and requests its separate process. When the native window loses focus, keep the title-bar background, divider, geometry, and menu availability unchanged while reducing the opacity of its menu labels, centered workspace identity, status actions, and window controls; restoring focus returns those elements to full opacity. Keep `desktop-shell-preview-focused=false` available to verify the inactive title bar without native window state.
