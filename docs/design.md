...
Long secondary collections should not make a primary action menu scroll. The workspace switcher keeps open-folder actions in its root menu, leaves the current-folder-location row text-only instead of repeating a folder glyph, and exposes older workspaces through a side-opening “Recent workspaces” submenu. The submenu must support pointer hover, click, and keyboard navigation, show complete paths with explicit WSL badges and no per-row icons, keep its scrollbar flush with the right edge, and reuse the shared menu density.
