...
**`application-mica-surface`** — A Windows-Mica-inspired content material for quiet grouped settings, currently used by the Agent memory options. Use the theme-aware `--mica-surface`, `--mica-border`, and `--mica-shadow` tokens with a 24px backdrop blur and restrained saturation. The material stays inside the web content layer: it must not require a transparent native window or change the WebView composition contract. Preserve text contrast and internal separators in both themes; do not add decorative color fields or motion to simulate wallpaper.
