...
- **Goal/Graph status panel:** newly created flows open their right-side status card automatically. The title-bar toggle collapses and expands that card with the same 180ms width curve as the conversation sidebar, leaving no interactive or visible track while collapsed.
