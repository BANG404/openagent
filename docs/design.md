...
The composer slash-command and mention palette uses the same compact row scale. Its floating surface follows the composer width and uses a 14px radius with a 6px inset. Keep the slash-command surface at a stable 320px outer height to match the add-item selector, while filtered mention results may shrink up to that same cap; both lists scroll independently so command discovery never pushes or resizes the composer.
