...
- **Desktop application shell:** keep one compact, continuous top chrome across
  ...
  The title bar, sidebar, and conversation stage remain fully transparent over
  the native window material: Windows prefers Mica with Acrylic and Blur
  fallbacks, while macOS uses Vibrancy. The main WebView shell adds no theme
  tint of its own.
  ...
  General Settings opens from the application menu as a centered dialog over
  the mounted chat shell; model, Agent, integration, memory, automation, and
  About management open in modeless singleton utility windows. The sidebar has
