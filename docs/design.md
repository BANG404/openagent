...
The composer slash-command and mention palette uses the same compact row scale. Its floating surface follows the composer width and uses a 14px radius with a 6px inset. Let both palettes size to their visible items up to a 320px outer maximum; only overflow scrolls independently, so short result sets have no trailing empty area and opening or navigating the list never moves the composer.
