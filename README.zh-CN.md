...
- **聊天组协作** — 在对话中提及选定角色即可唤醒它们参与讨论；角色回复会保留在聊天组线程中，也可在不离开当前对话的情况下发送 Agent 间私信。
...
- **原生 MCP 集成** — 通过 HTTP 或 stdio 连接外部 MCP 服务器，查看缓存的服务能力目录，并在调用时通过工具搜索发现相关的分组工具。
- **开箱即用的开发工具** — 内置补丁、图片检查与终端工具。受管终端会话支持交互式或长时间运行的后台进程。
- **桌面自动化** — 内置 Cua Driver 由桌面宿主监管运行，并在顶栏**集成 → 插件**面板中提供产品固定的权限策略与逐工具开关。
- **插件控制** — 顶栏**集成 → 插件**入口会打开独立的插件设置窗口，以可展开卡片管理内置能力：开关聊天组协作，测试 Cua Driver MCP 连接，发现其工具，并选择向 Agent 暴露哪些桌面操作。
...
- 右侧对话面板只在存在 Goal/Graph 状态、文件变更或当前对话拥有的后台终端时出现。
- 原通用 Agent Plugins 管理界面不再开放。技能与用户配置的 MCP 服务保留各自的设置入口，“插件”界面管理内置聊天组与 Cua Driver 控制项。
