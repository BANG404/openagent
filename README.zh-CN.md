...
- [通过工具调用渲染网页预览 (`render_web`)](#通过工具调用渲染网页预览-render_web)
- [记忆文件格式](#记忆文件格式)
...
- **通过工具调用渲染网页预览 (`render_web`)** — Agent 可在对话流中直接显示本地 HTML 文件或可交互的 HTTP(S) 网站，支持全屏与使用系统浏览器打开；本地文件仍可复制内容和导出 PNG。
...
<a id="通过工具调用渲染网页预览-render_web"></a>

## 通过工具调用渲染网页预览 (`render_web`)

Agent 不再使用行内 AGUI 标签，而是调用 `render_web` 并传入工作区内的 `.html`/`.htm` 文件路径或 HTTP(S) 地址。本地文件通过工作区范围的预览协议解析相邻资源，支持复制内容和尽力导出 PNG；远程网站在沙箱 Frame 中保留脚本与表单交互，支持全屏和一键用系统浏览器打开，但网站自身的 CSP 或 `X-Frame-Options` 仍可能拒绝嵌入。
...
- **自动 Agent 记忆检索**：默认关闭，由 Chat Agent 判断当前对话是否需要调用 `search_agent_memory` 检索相关记忆。开启后，每次对话都会先使用 Flash 模型将当前消息改写为聚焦查询，检索相关的结构化记忆，并将结果加入 Chat Agent 的系统提示词。
