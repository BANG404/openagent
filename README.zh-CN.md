...
- **每个工作区独立窗口** — 可同时打开多个项目，每个窗口维护自己的活动工作区；再次选择已打开的工作区时会聚焦原窗口。
- **可复用委派角色** — 可在全局或项目范围保存专业 Agent 工作流，按职责搜索，并派发为实时输出的子对话。
- **更智能的技能发现** — 使用 `metadata.category` 为技能分组；未分类技能可由 Flash 任务自动归类，让大型技能目录按需渐进展开，不再占满每次提示词。
- **持久化多模态分支** — 图片、PDF 与文本附件支持丰富预览，可跨重载与检查点恢复；编辑消息创建新分支时仍会保留附件。
- **长对话更流畅** — 对话搜索与分页、已完成历史虚拟化、稳定流式消息、后续消息排队和可取消压缩，让大型历史记录保持响应。
- **更丰富的工具输出** — Mermaid 校验渲染、AGUI 图片/视频媒体、源代码行预览与受管后台终端，让长时间运行和可视化任务更易跟踪。
...
- **混合长期记忆** — SQLite + FTS5 + 384 维向量嵌入（fastembed `AllMiniLML6V2`），结合时间衰减权重实现跨会话精准召回。检索前可选用 Flash 任务将最新消息改写为聚焦的语义查询，使记忆匹配意图而不只依赖原始措辞。
