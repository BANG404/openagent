<script lang="ts">
  ...
  let query = $state("");
  let items = $derived([
    { value: defaultRoleKey, label: $t("defaultRoleName"), description: $t("defaultRoleDescription") },
    ...roles.map((role) => ({
      value: role.id,
      label: role.name,
...
<style>
  ...
  .role-selector-copy { min-width: 0; flex: 1; display: flex; align-items: baseline; gap: 5px; font-size: 13px; }
  .role-selector-label { flex: none; color: var(--text); }
  .role-selector-name { min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; font-size: 13px; }
  .role-selector-caret { width: 13px; height: 13px; flex: 0 0 13px; color: var(--text-muted); }
  .role-selector-caret path { fill: none; stroke: currentColor; stroke-width: 1.5; stroke-linecap: round; stroke-linejoin: round; }
  :global(.role-selector-trigger.compact .role-selector-caret) { display: none; }
  ...
  .role-selector-search-wrap { position: relative; padding: 2px 2px 6px; }
  .role-selector-search-wrap svg { position: absolute; width: 14px; height: 14px; left: 12px; top: 12px; stroke: var(--text-muted); stroke-width: 1.5; stroke-linecap: round; pointer-events: none; }
  .role-selector-search-wrap input { width: 100%; box-sizing: border-box; border: 0; border-radius: 6px; padding: 7px 10px 7px 30px; background: var(--surface2); color: var(--text); font: inherit; font-size: 13px; outline: none; box-shadow: var(--control-shadow); }
  :global(.role-selector-viewport) { min-height: 0; flex: 1; overflow-y: auto; }
  :global(.role-selector-item) { display: flex; align-items: center; gap: 8px; padding: 7px 9px; border-radius: 6px; color: var(--text); outline: none; cursor: pointer; }
  ...
  .role-selector-empty { padding: 12px; color: var(--text-muted); font-size: 12px; text-align: center; }
  .role-selector-item-copy { min-width: 0; flex: 1; display: grid; gap: 2px; }
  .role-selector-item-name { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; font-size: 13px; }
  .role-selector-item-description { display: -webkit-box; overflow: hidden; color: var(--text-muted); font-size: 11px; line-height: 1.35; -webkit-box-orient: vertical; -webkit-line-clamp: 2; line-clamp: 2; }
  .role-selector-check { width: 14px; height: 14px; flex: 0 0 14px; color: var(--primary); }
  .role-selector-check path { fill: none; stroke: currentColor; stroke-width: 1.75; stroke-linecap: round; stroke-linejoin: round; }
</style>
