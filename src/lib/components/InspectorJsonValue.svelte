<script lang="ts">
  ...
  let {
    ...
    label = "value",
    depth = 0,
  }: { value: unknown; label?: string; depth?: number } = $props();
  ...
  const summary = $derived(
    Array.isArray(value)
      ...
      : isObject(value)
        ? `Object · ${Object.keys(value).length} field${Object.keys(value).length === 1 ? "" : "s"}`
        : "",
  );
    ...
    <div class="json-children">
      {#each childEntries as [key, child] (key)}
        <div class="json-child">
          <InspectorJsonValue value={child} label={key} depth={depth + 1} />
        </div>
      {/each}
    </div>
  </details>
