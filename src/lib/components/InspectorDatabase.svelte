<script lang="ts">
  ...
  const selectedSummary = $derived(
    overview?.tables.find((table) => table.name === selectedTable) ?? null,
  );
  const pageStart = $derived(tableData && tableData.total_rows ? tableData.offset + 1 : 0);
  const pageEnd = $derived(
    tableData ? Math.min(tableData.offset + tableData.rows.length, tableData.total_rows) : 0,
  );
  const selectedColumn = $derived(
    selectedCell && tableData ? (tableData.columns[selectedCell.columnIndex] ?? null) : null,
  );
  const selectedValue = $derived(
    selectedCell && tableData
      ? (tableData.rows[selectedCell.rowIndex]?.[selectedCell.columnIndex] ?? "")
      : "",
  );
  const selectedJson = $derived(parseJson(selectedValue));
  const checkpointMessages = $derived(getCheckpointMessages(selectedJson));
  const filteredCheckpointMessages = $derived(
    checkpointMessages?.filter((message) =>
      checkpointMessageSearchText(message).includes(cellSearch.trim().toLocaleLowerCase()),
    ) ?? [],
  );
  ...
  function checkpointMessageSearchText(message: Record<string, unknown>): string {
    return [
      message.role,
      checkpointContentType(message),
      checkpointContentText(message.content),
  ...
  function timestampDisplay(value: string, columnName: string): string | null {
    if (
      !/(?:^|_)(?:created|updated|accessed|confirmed)_at$/i.test(columnName) ||
      !/^\d+$/.test(value)
    )
      return null;
  ...
  function cellPreview(value: string, columnName = ""): string {
    ...
    if (parsed && typeof parsed === "object")
      return `{JSON object · ${Object.keys(parsed).length}}`;
...
<section
  class:detail-open={selectedCell !== null}
  class="database-view"
...
>
  <aside class="table-sidebar">
    <div class="panel-heading">
      <div>
        <p class="eyebrow">SQLITE TABLES</p>
        <h2>{overview?.tables.length ?? 0} tables</h2>
      </div>
  ...
  <section class="data-panel">
    <div class="data-heading">
      <div>
        ...
        <p class="muted">
          {selectedSummary
            ? `${selectedSummary.row_count.toLocaleString()} total rows`
            : "Browse persisted application data and schema"}
        </p>
      </div>
      <form role="search" onsubmit={submitSearch}>
        <input
          bind:value={searchInput}
          aria-label="Search selected table"
          placeholder="Search all fields…"
          disabled={!selectedTable || loading}
        />
        {#if activeSearch}<button type="button" onclick={clearSearch} disabled={loading}
            >Clear</button
          >{/if}
    ...
    {:else if tableData}
      <details class="schema" open>
        ...
        <div class="schema-grid">
          {#each tableData.columns as column (column.cid)}
            <article>
              <div>
                <code>{column.name}</code>{#if column.primary_key}<span class="badge">PK</span
                  >{/if}{#if column.not_null}<span class="badge">NOT NULL</span>{/if}
              </div>
              <span
                >{column.data_type || "untyped"}{column.default_value === null
                  ? ""
                  : ` · default ${column.default_value}`}</span
              >
            </article>
          {/each}
        </div>
      ...
      <div class="result-meta">
        <span
          >{activeSearch
            ? `${tableData.total_rows.toLocaleString()} matches for “${activeSearch}”`
            : `${tableData.total_rows.toLocaleString()} rows`}</span
        >
        ...
        <table>
          ...
          <tbody>
            {#each tableData.rows as row, rowIndex (`${tableData.offset}-${rowIndex}`)}
              <tr
                >{#each row as value, columnIndex (columnIndex)}<td
              ...
              >
            {:else}<tr
                ><td class="empty-cell" colspan={Math.max(tableData.columns.length, 1)}
                  >No rows found.</td
                ></tr
              >{/each}
          </tbody>
      ...
      <div class="pagination">
        ...
        <button
          onclick={nextPage}
          disabled={loading || tableData.offset + tableData.rows.length >= tableData.total_rows}
          >Next</button
        >
      </div>
    {/if}
  ...
  {#if selectedCell && tableData && selectedColumn}
    <aside class="row-detail" aria-label="Selected database cell">
      <div class="panel-heading">
        <div>
          <p class="eyebrow">CELL PREVIEW</p>
          <h2>{tableData.table}.{selectedColumn.name}</h2>
          <p class="muted">
            {selectedColumn.data_type || "untyped"} · row {(
              tableData.offset +
        ...
        <button onclick={() => (selectedCell = null)}>Close</button>
      </div>
      <div class="cell-preview">
        {#if timestampDisplay(selectedValue, selectedColumn.name)}
          <div class="timestamp-preview">
            <strong>{timestampDisplay(selectedValue, selectedColumn.name)}</strong><span
              >Unix timestamp: <code>{selectedValue}</code></span
            >
          </div>
          ...
          <div class="checkpoint-preview-heading">
            <div>
              <strong>Messages</strong><span
                >{filteredCheckpointMessages.length} of {checkpointMessages.length}</span
              >
            </div>
            <input
              bind:value={cellSearch}
              type="search"
              aria-label="Search checkpoint messages"
              placeholder="Search role, type, or content"
            />
          </div>
              ...
              <details class="checkpoint-message" open={filteredCheckpointMessages.length <= 8}>
                ...
                <div class="checkpoint-message-body">
                  <div class="message-meta">
                    <span>role <code>{String(message.role ?? "unknown")}</code></span><span
                      >content type <code>{checkpointContentType(message)}</code></span
                    >
                  </div>
...
<style>
  ...
  .database-view.detail-open {
    grid-template-columns: 250px minmax(0, 1fr) minmax(300px, 38%);
  }
  .table-sidebar,
  .data-panel {
    display: flex;
  ...
  .checkpoint-preview-heading > div,
  .message-meta {
    display: flex;
    align-items: center;
    justify-content: space-between;
  ...
  .checkpoint-preview-heading strong {
    font-size: 12px;
  }
  .checkpoint-preview-heading span {
    color: var(--text-muted);
    font-size: 11px;
  }
  .checkpoint-preview-heading input {
    width: 100%;
  }
  .checkpoint-messages {
    display: grid;
  ...
  .checkpoint-message summary {
    display: grid;
    min-width: 0;
    max-width: 100%;
    overflow: hidden;
    grid-template-columns: auto auto max-content minmax(0, 1fr);
    align-items: center;
    gap: 7px;
    padding: 8px;
    cursor: pointer;
  ...
  .checkpoint-message summary::-webkit-details-marker {
    display: none;
  }
  .message-index {
    color: var(--text-muted);
  ...
  .checkpoint-message summary > code,
  .message-meta code {
    color: var(--primary);
    font:
      10px ui-monospace,
  ...
  .checkpoint-message-body pre {
    max-height: 260px;
  }
  .checkpoint-raw {
    margin-top: 10px;
  }
  .checkpoint-raw summary {
    cursor: pointer;
    color: var(--text-muted);
    font-size: 11px;
  }
  .checkpoint-raw :global(.json-container) {
    margin-top: 8px;
  }
  .timestamp-preview {
    display: grid;
